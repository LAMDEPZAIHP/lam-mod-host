...
// =========================
// OBFUSCATE SIÊU MẠNH + CODE RÁC SIÊU NHIỀU (300 ~ 8000 dòng)
// Dùng table assignment để tránh giới hạn 200 local của Lua
// =========================
function obfuscateLuaCode(codeText) {
  // XOR + Base64 + Reverse (đã test chạy ổn với Lua 5.3 / executor)
  ...
  const chunkSize = 22 + Math.floor(Math.random() * 35);
  const chunks = [];
  ...
  // ========== CODE RÁC SIÊU NHIỀU (300 ~ 8000) ==========
  // Quan trọng: KHÔNG dùng hàng nghìn "local xxx =" vì Lua giới hạn ~200 local/function
  // Thay bằng 1 table + hàng nghìn assignment → không bị limit
  ...
  for (let i = 0; i < numJunk; i++) {
    ...
    if (t === 0) {
      ...
      junkLines.push(
        `${v.J}[${key}] = "${crypto.randomBytes(4 + (i % 6)).toString("hex")}"`
      );
    } else if (t === 2) {
      junkLines.push(
        `${v.J}[${key}] = {${Math.floor(Math.random() * 90)}, ${Math.floor(Math.random() * 90)}, "${crypto.randomBytes(2).toString("hex")}"}`
      );
    } else if (t === 3) {
      junkLines.push(
        `${v.J}[${key}] = function() return ${Math.floor(Math.random() * 5000)} end`
      );
    } else if (t === 4) {
      junkLines.push(
        `if false then ${v.J}[${key}] = ${Math.floor(Math.random() * 100)} end`
      );
    } else if (t === 5) {
      junkLines.push(
        `${v.J}[${key}] = (${Math.floor(Math.random() * 40)} * ${Math.floor(Math.random() * 40)} + ${Math.floor(Math.random() * 10)}) % 997`
      );
    } else if (t === 6) {
      junkLines.push(
        `${v.J}[${key}] = function(x) return type(x) == "number" and x * 0 or 0 end`
      );
    } else if (t === 7) {
      junkLines.push(
        `for _ = 1, ${1 + Math.floor(Math.random() * 2)} do ${v.J}[${key}] = _ end`
      );
    } else if (t === 8) {
      junkLines.push(
        `${v.J}[${key}] = string.rep("${String.fromCharCode(97 + (i % 26))}", ${1 + (i % 3)})`
      );
    } else if (t === 9) {
      junkLines.push(
        `${v.J}[${key}] = {{${Math.floor(Math.random() * 9)}}, {${Math.floor(Math.random() * 9)}, ${Math.floor(Math.random() * 9)}}}`
      );
    } else {
      junkLines.push(
        `${v.J}[${key}] = "${crypto.randomBytes(2).toString("hex")}" .. "${crypto.randomBytes(2).toString("hex")}"`
      );
    }
  }

  // Thêm vài function rác (có local bên trong — an toàn)
  for (let i = 0; i < 15; i++) {
    const fn = unique();
    const a = unique();
    const inner = unique();
  ...
  return `--[[
  🔒 LÂM MOD — ULTRA PROTECT v3
  XOR + Base64 + Reverse + ~${numJunk} junk lines
