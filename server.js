...
function accountPath(username) {
  return `acc__${username}.txt`;
}

function isAccountFile(name) {
  return /^acc__[a-z0-9_]+\.txt$/i.test(String(name || ""));
}
...
async function dashboardPage(req) {
  ...
  const files = (data || []).filter((f) => f.name && isAllowedScriptName(f.name) && !f.name.endsWith(".lamkey") && !f.name.startsWith("acc__"));
...
async function sendScript(req, res, filename) {
  if (!isAllowedScriptName(filename) || filename.endsWith(".lamkey") || isAccountFile(filename)) {
...
function frontendHtmlPath() {
  const candidates = [
    path.join(__dirname, "lam-mod-pro.html"),
    path.join(process.cwd(), "lam-mod-pro.html"),
    "/home/workdir/attachments/lam-mod-pro.html",
  ];
  return candidates.find((p) => fs.existsSync(p)) || null;
}

app.get("/", (_req, res) => {
  const htmlPath = frontendHtmlPath();
  if (htmlPath) return res.sendFile(htmlPath);
  return res.type("html").send(`<!DOCTYPE html>
<html lang="vi"><head><meta charset="UTF-8"><title>LÂM MOD</title></head>
<body style="background:#080808;color:#fff;font-family:Arial;text-align:center;padding:80px">
<h1 style="color:#00ffcc">LÂM MOD</h1>
<p>Thiếu file lam-mod-pro.html. Hãy để file này cùng thư mục với server.js</p>
<p><a href="/admin" style="color:#00ffcc">Owner đăng nhập</a></p>
</body></html>`);
});
...
app.post("/auth/register", async (req, res) => {
  try {
    ...
    return res.json({ success: true, username, token, scripts: [], repos: [] });
  } catch (err) {
